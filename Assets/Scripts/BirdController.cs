...
public class BirdController : MonoBehaviour
{
    ...
    void Die()
    {
        ...
        // Сохраняем скорость полёта (инерция)
        Vector3 deathVelocity = lastMoveDirection * currentSpeed;
        ...
        rb.useGravity = true;
        rb.linearVelocity = deathVelocity;           // продолжаем лететь вперёд
        rb.angularVelocity = Random.insideUnitSphere * 3f; // хаотичное вращение

        // Небольшой случайный разброс, чтобы тарелки вели себя по-разному
        rb.linearVelocity += Random.insideUnitSphere * 1.5f;

        // Уничтожаем через 3 секунды (тонет в океане)
